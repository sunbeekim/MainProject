...
const MessageInput: React.FC<MessageInputProps> = ({ onSendMessage }) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {//enter key입력 메시지 전송
      e.preventDefault();
      handleSend();
    }
  };
    ...
    <div className="flex flex-col border-t p-2 bottom-1">
      ...
      {file && (
        <div className="relative w-40 mb-2">
          {/* 이미지 파일 미리보기 */}
          {file.type === "image" && (
            <img src={file.url} alt="이미지 미리보기" className="w-full h-32 object-cover rounded-md shadow-md" />
          )}
          {/* 비디오 파일 미리보기 */}
          {file.type === "video" && (
      ...
      <div className="flex items-center fixed bottom-0 left-0 right-0 bg-white shadow-md p-7">
        ...
        <button 
          onClick={handleSend}
          className="bg-[#FBCCC5] hover:bg-[#F9B0BA] text-white px-4 py-2 rounded-full"
        >
          전송
        </button>
      </div>
